...
class LocationAnalyzer:
    ...
    def __init__(self, google_maps_api_key: str = None, gemini_api_key: str = None):
        """
    ...
    def find_nearby_places(self, lat: float, lon: float, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for place in places_result.get('results', []):
                ...
                a = math.sin(lat_diff/2)**2 + math.cos(math.radians(lat)) * math.cos(math.radians(place_lat)) * math.sin(lon_diff/2)**2
    ...
    def get_place_info(self, lat: float, lon: float) -> Optional[Dict]:
        ...
        return {
            ...
            'place_name': f"Location at {lat:.4f}°N, {lon:.4f}°W" if lon < 0 else f"Location at {lat:.4f}°N, {lon:.4f}°E",
            'types': [location_type],
            'address_context': {
    ...
    def analyze_with_gemini(self, place_info: Dict, user_search_context: str = "", nearby_places: List[Dict] = None) -> str:
        """
        ...
        analysis_parts = [
            f"📍 Geographic Location: {lat:.4f}°N, {lon:.4f}°W" if lon < 0 else f"📍 Geographic Location: {lat:.4f}°N, {lon:.4f}°E",
            ...
            for i, place in enumerate(nearby_places, 1):
                ...
                if distance_m is not None:
                    if distance_m < 1000:
                        ...
                        analysis_parts.append(f"     Distance: {distance_m/1000:.2f}km")
                if address:
        ...
        if user_search_context and nearby_places:
            analysis_parts.append(f"\n{'='*80}")
            analysis_parts.append(f"🎯 COMMODITY ANALYSIS: {user_search_context.upper()}")
            analysis_parts.append(f"{'='*80}")
            analysis_parts.append("Looking for FIRST POINTS OF AGGREGATION (processing facilities)")
            analysis_parts.append("where raw commodities are transported to and processed.")
            analysis_parts.append("")
            # AI-powered analysis of whether nearby places are processing facilities
            relevance_analysis = self._analyze_search_relevance(nearby_places, user_search_context)
    ...
    def _analyze_search_relevance(self, nearby_places: List[Dict], search_context: str) -> str:
        """
        ...
        try:
            ...
            for i, place in enumerate(nearby_places, 1):
                ...
                rating = place.get('rating', 'N/A')
                distance_m = place.get('distance_m', None)
                distance_str = f"{distance_m:.0f}m" if distance_m and distance_m < 1000 else f"{distance_m/1000:.2f}km" if distance_m else "unknown"
                
                places_info.append(f"{i}. {name} (Distance: {distance_str})\n   Types: {types}\n   Address: {address}\n   Rating: {rating}")
            ...
            examples = commodity_examples.get(search_context.lower(), "processing facilities, mills, warehouses, factories")
...
"""
    ...
    def analyze_location(self, lat: float, lon: float, user_search_context: str = "") -> Dict:
        """
        ...
        try:
            ...
            if nearby_places:
                analysis = self.analyze_with_gemini(place_info, user_search_context, nearby_places)
            else:
...
def create_location_analyzer(google_maps_api_key: str = None, gemini_api_key: str = None) -> Optional[LocationAnalyzer]:
    """
