...
def find_band_files(
    mgrs_tile_id: str, bands: List[str], date_range: str, local_dir: str
) -> List[str]:
    """
    ...
    for band in bands:
        patterns = [
            ...
            f"{mgrs_tile_id}_B{band[-1]}_{date_range}.tif"
            if band.startswith("B")
            else None,
            f"{mgrs_tile_id}_B{band[-2:]}_{date_range}.tif"
            if band.startswith("B") and len(band) > 2
            else None,
        ]
...
def create_stacked_raster(
    band_paths: List[str], output_path: str, target_resolution: int = 10
) -> str:
    """
    ...
    with rasterio.open(band_paths[0]) as template:
        ...
        if template.res[0] != target_resolution:
            ...
            print(
                f"  Scaling from {template.res[0]}m to {target_resolution}m (factor: {scale_factor:.3f})"
            )
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        for i, band_path in enumerate(band_paths):
            with rasterio.open(band_path) as src:
                if geovibes.res[0] != target_resolution:
                    scale_factor = geovibes.res[0] / target_resolution
                    new_width = int(geovibes.width * scale_factor)
                    new_height = int(geovibes.height * scale_factor)
                    data = geovibes.read(
                        1,
...
def get_raster_info(raster_path: str) -> Dict[str, Any]:
    ...
    with rasterio.open(raster_path) as src:
        return {
            "crs": geovibes.crs,
            "bounds": geovibes.bounds,
            "width": geovibes.width,
            "height": geovibes.height,
            "resolution": geovibes.res,
            "count": geovibes.count,
            "dtype": geovibes.dtypes[0] if geovibes.count > 0 else None,
            "transform": geovibes.transform,
        }
