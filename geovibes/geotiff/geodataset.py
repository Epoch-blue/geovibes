...
class GeotiffTileDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        with Env(**self.env_kwargs):
            with rasterio.open(path) as src:
                self.width, self.height = geovibes.width, geovibes.height
                self.crs = geovibes.crs
                self.transform = geovibes.transform
                self.count = geovibes.count
                self.native_resolution = geovibes.res[0]
    ...
    def _precalculate_windows(self):
        ...
        logger.info(
            f"Pre-calculating windows for {len(self.geometries_gdf)} geometries..."
        )
        ...
        with Env(**self.env_kwargs):
            with rasterio.open(self.path) as src:
                for idx in range(len(self.geometries_gdf)):
                    if idx % 1000 == 0:
                        logger.info(
                            f"Pre-calculating window {idx}/{len(self.geometries_gdf)}"
                        )
        ...
        try:
            with Env(**self.env_kwargs):
                with rasterio.open(self.path) as src:
                    ...
                    data = geovibes.read(
                        boundless=False, window=window, fill_value=0, masked=False
                    )
                    ...
                    resolution = geovibes.res[0]
                    expected_width = int(width_meters / resolution)
                    expected_height = int(height_meters / resolution)

                    logger.info(
                        f"Raster info: resolution={resolution}m, CRS={geovibes.crs}"
                    )
                    logger.info(
                        f"Geometry bounds: {geom_bounds} (width={width_meters:.1f}m, height={height_meters:.1f}m)"
                    )
                    logger.info(
                        f"Expected size at {resolution}m: {expected_height}x{expected_width} pixels"
                    )
                    logger.info(
                        f"Window: row_off={window.row_off}, col_off={window.col_off}, height={window.height}, width={window.width}"
                    )
                    logger.info(
                        f"Native tile size for first geometry: {data.shape} (channels, height, width)"
                    )
    ...
    def read_window_from_geometry(
        ...
        with Env(**self.env_kwargs):
            with rasterio.open(self.path, "r") as src:
                ...
                if (
                    ...
                    and self.native_resolution != self.target_resolution
                ):
                ...
                needs_padding = (read_height < expected_height - 1) or (
                    read_width < expected_width - 1
                )
                ...
                if current_h != expected_height or current_w != expected_width:
                    final_data = np.zeros(
                        (self.count, expected_height, expected_width), dtype=data.dtype
                    )
        ...
        geom_info = {
            ...
            "window": clipped_window,
            "bounds_pixel": (
                clipped_window.col_off,
                clipped_window.row_off,
                clipped_window.col_off + clipped_window.width,
                clipped_window.row_off + clipped_window.height,
            ),
        }

        logger.debug(
            f"Successfully read and padded window for geometry {index}, final data shape: {data.shape}"
        )
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | int | str]:
        ...
        logger.debug(
            f"Transform application for index {index}: {transform_elapsed:.4f}s"
        )
        ...
        if total_elapsed > 0.5:  # Log slow items
            logger.warning(
                f"Slow __getitem__ for index {index}: {total_elapsed:.3f}s total"
            )
        else:
            logger.debug(
                f"Completed __getitem__ for index {index}: {total_elapsed:.4f}s total"
            )
