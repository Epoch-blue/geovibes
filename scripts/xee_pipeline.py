...
from geovibes.database.xee_embeddings import (
    ...
    load_roi_geometry,
    get_alphaearth_embeddings,
    export_embeddings_to_geotiff,
)
...
def run_xee_pipeline(
    ...
    if faiss_only:
        ...
        print(
            f"📈 Database stats: {stats['total_rows']:,} rows, {stats['embedding_dimension']} dimensions"
        )
        ...
        memory_safe_batch_size = (
            50_000  # Very conservative batch size for memory safety
        )
        print(
            f"Using memory-safe batch size: {memory_safe_batch_size:,} vectors per batch"
        )
        create_faiss_index(
            str(db_path),
            str(index_path),
    ...
    if geotiff_mode:
        if geotiff_path:
            ...
            total_ingested = 0
            global_id_counter = 0
            for batch_df in read_geotiff_streaming(
                geotiff_path, tile_id=db_name, batch_size=batch_size
            ):
                ingested = stream_ingest_dataframe(
                    str(db_path),
                    batch_df,
            ...
            if not bucket or not prefix:
                raise ValueError(
                    "Must provide --bucket and --prefix for GeoTIFF export mode"
                )
            ...
            export_embeddings_to_geotiff(
                image, roi_ee, scale, bucket, prefix, start_date, end_date
            )
            ...
            return {
                ...
                "bucket": bucket,
                "prefix": prefix,
                "message": "Export task queued - wait for completion then run ingest phase with --geotiff-path",
            }
    ...
    total_ingested = stream_ingest_dataframe(
        str(db_path), df, batch_size=batch_size, embedding_col="embedding"
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default=".", help="Output directory for database files"
    )
    ...
    parser.add_argument(
        "--scale", type=int, default=100, help="GEE resolution in meters (default: 100)"
    )
    ...
    parser.add_argument(
        "--m", type=int, default=64, help="FAISS PQ m parameter (default: 64)"
    )

    parser.add_argument(
        "--nbits", type=int, default=8, help="FAISS PQ nbits parameter (default: 8)"
    )
    ...
    if args.faiss_only:
        ...
        parser.error(
            "Must provide either --roi-file/--bbox (for export) or --geotiff-path (for ingest)"
        )
