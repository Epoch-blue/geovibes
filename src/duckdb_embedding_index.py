...
def create_database(
    ...
    path_strings = [
        f"'{p_str}'"
        for p_str in (str(p.resolve()).replace("\\", "/") for p in parquet_files)
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "input_dir", type=str, help="Directory containing input Parquet files"
    )
    ...
    parser.add_argument(
        "--metric",
        type=str,
        choices=["l2sq", "cosine", "inner_product"],
        default="cosine",
        help="Distance metric for HNSW index (default: cosine)",
    )
